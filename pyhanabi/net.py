...
class PublicLSTMPolicyNet(torch.jit.ScriptModule):
    ...
    def act(
        ...
        assert priv_s.dim() == 2
        publ_s = priv_s[:, -self.publ_in_dim :]
        ...
        v = self.fc_v(o).squeeze(2).squeeze(0)
        ...
        assert v.size() == action.size()

        return action, log_pa, v, {"h0": h, "c0": c}

    @torch.jit.script_method
    def act_with_log_probs(
        self,
        priv_s: torch.Tensor,
        legal_move: torch.Tensor,
        hid: Dict[str, torch.Tensor],
        ...
        publ_o, (h, c) = self.lstm(x, (hid["h0"], hid["c0"]))
        priv_o = self.priv_net(priv_s)
        a = self.fc_a(priv_o * publ_o).squeeze(0)
        legal_a = a - (1 - legal_move) * 1e10
        log_pa = nn.functional.log_softmax(legal_a, 1).squeeze(0)
        if self.training:
            prob_a = nn.functional.softmax(legal_a, 1)
            action = prob_a.multinomial(1).squeeze(1)
        else:
    ...
    def forward(
        ...
        log_p = nn.functional.log_softmax(logit_a, 2)
