...
using namespace infra;
  ...
  void EpisodeBuffer::pushObs(const TensorDict& obs) {
    assert(callOrder_ == 0);
    ++callOrder_;
  ...
  void EpisodeBuffer::pushAction(const TensorDict& action) {
    assert(callOrder_ == 1);
    ++callOrder_;
  ...
  void EpisodeBuffer::pushReward(float r) {
    assert(callOrder_ == 2);
    ++callOrder_;
    reward_[seqLen_] = r;
  }

  void EpisodeBuffer::pushTerminal(float t) {
    assert(callOrder_ == 3);
    callOrder_ = 0;
    terminal_[seqLen_] = t;
    ++seqLen_;
  ...
  RNNTransition EpisodeBuffer::popTransition() {
    assert(callOrder_ == 0);
