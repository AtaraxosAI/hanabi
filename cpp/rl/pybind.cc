...
PYBIND11_MODULE(hanalearn, m) {
  ...
  py::class_<RLActor, std::shared_ptr<RLActor>>(m, "RLActor")
      .def(
          py::init<
          ...
          py::arg("gamma"))
      .def(py::init<
           std::shared_ptr<infra::BatchRunner>,
           int,      // numPlayer
           int,      // playerIdx
           bool,     // vdn
           bool,     // sad
           bool>())  // hideAction
      .def("set_partners", &RLActor::setPartners)
      .def("set_explore_eps", &RLActor::setExploreEps)
      .def("set_boltzmann_t", &RLActor::setBoltzmannT)
      .def("get_success_fict_rate", &RLActor::getSuccessFictRate)
      .def("get_played_card_info", &RLActor::getPlayedCardInfo);
  ...
  py::class_<HanabiThreadLoop, infra::ThreadLoop, std::shared_ptr<HanabiThreadLoop>>(
      m, "HanabiThreadLoop")
      .def(py::init<
           std::vector<std::shared_ptr<HanabiEnv>>,
           std::vector<std::vector<std::shared_ptr<RLActor>>>,
           bool>())
  ...
  // search related
  m.def("search_observe", &searchObserve);
  m.def("filter_sample", &search::filterSample);
  m.def("search_move", &search::searchMove);
  m.def("parallel_search_moves", &search::parallelSearchMoves);
//   m.def("parallel_search_moves", &search::parallelSearchMoves, py::call_guard<py::gil_scoped_release>());

  py::class_<search::Player, std::shared_ptr<search::Player>>(
    m, "SimPlayer")
    .def(py::init<int, std::shared_ptr<infra::BatchRunner>, infra::TensorDict>());
