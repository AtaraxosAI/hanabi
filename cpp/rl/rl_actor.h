...
#include "cpp/infra/batch_runner.h"
#include "cpp/infra/replay.h"
#include "cpp/infra/episode_buffer.h"

#include "cpp/rl/hanabi_env.h"
#include "cpp/utils.h"

class RLActor {
 public:
  RLActor(
      std::shared_ptr<infra::BatchRunner> runner,
      int seed,
      ...
      std::shared_ptr<infra::Replay> replayBuffer,
      ...
      , episodeBuffer_(std::make_unique<infra::EpisodeBuffer>(multiStep, seqLen, gamma)) {
  ...
  RLActor(
      ...
      , episodeBuffer_(nullptr) {
    initHidden_ = getH0(1, runner_);
  }

  void setPartners(std::vector<std::shared_ptr<RLActor>> partners) {
  ...
  void setBeliefRunner(std::shared_ptr<infra::BatchRunner>& beliefModel) {
 ...
 private:
  infra::TensorDict getH0(int numPlayer, std::shared_ptr<infra::BatchRunner>& runner) {
    std::vector<torch::jit::IValue> input{numPlayer};
    auto model = runner->jitModel();
    auto output = model.get_method("get_h0")(input);
    auto h0 = infra::tensor_dict::fromIValue(output, torch::kCPU, true);
    return h0;
  }
  ...
  std::shared_ptr<infra::BatchRunner> runner_;
  std::mt19937 rng_;
  ...
  // optional, e.g. RL agent does not use it
  ...
  std::shared_ptr<infra::Replay> replayBuffer_;
  std::unique_ptr<infra::EpisodeBuffer> episodeBuffer_;

  infra::TensorDict initHidden_;
  infra::TensorDict prevHidden_;
  infra::TensorDict hidden_;

  infra::RNNTransition lastEpisode_;

  bool offBelief_ = false;
  std::shared_ptr<infra::BatchRunner> beliefRunner_;
  infra::TensorDict beliefHidden_;
  ...
  std::vector<std::shared_ptr<RLActor>> partners_;

  // to control stages
  Stage stage_ = Stage::ObserveBeforeAct;
  std::unordered_map<std::string, infra::FutureReply> futures_;
