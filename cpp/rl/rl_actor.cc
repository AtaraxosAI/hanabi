...
infra::RNNTransition stackForVDN(const std::vector<infra::RNNTransition>& taus) {
  assert(taus.size() > 1);

  infra::RNNTransition tauJoint;
  ...
  std::vector<infra::TensorDict> allObs;
  std::vector<infra::TensorDict> allAction;
  std::vector<infra::TensorDict> allH0;
  ...
  tauJoint.obs = infra::tensor_dict::stack(allObs, 1);
  tauJoint.action = infra::tensor_dict::stack(allAction, 1);
  tauJoint.h0 = infra::tensor_dict::cat(allH0, 1);
...
void RLActor::reset(const HanabiEnv& env) {
...
std::unique_ptr<hle::HanabiMove> RLActor::next(const HanabiEnv& env) {
//   // backpressure: don't do more work while the replay is full
//   if (replayBuffer_ != nullptr) {
//     replayBuffer_->waitIfFull();
//   }
...
void RLActor::observeBeforeAct(const HanabiEnv& env) {
  ...
  auto [beliefInput, privCardCount, v0] = searchObserve(
      state, playerIdx_);
...
std::unique_ptr<hle::HanabiMove> RLActor::decideMove(const HanabiEnv& env) {
  torch::NoGradGuard ng;
  ...
  if (replayBuffer_ != nullptr) {
    episodeBuffer_->pushAction(reply);
  }
...
void RLActor::fictAct(const HanabiEnv& env) {
  if (!offBelief_) {
    return;
  }
...
void RLActor::observeAfterAct(const HanabiEnv& env) {
  if (replayBuffer_ == nullptr) {
    return;
  }
  ...
  episodeBuffer_->pushReward(reward);
  episodeBuffer_->pushTerminal(float(terminated));

  if (offBelief_) {
    assert(false);
    // assert(futures_.size() == 1);
    // auto target = getResultAndErase("target", futures_).at("target");
    // episodeBuffer_->obsBack().emplace("target", target);
    // episodeBuffer_->obsBack().emplace("valid_fict", torch::tensor(float(validFict_)));
  }
  assert(futures_.size() == 0);
  ...
  if (vdn_) {
    ...
    std::vector<infra::RNNTransition> taus;
    assert(numPlayer_ == (int)partners_.size());
    for (int i = 0; i < numPlayer_ - 1; ++i) {
      taus.push_back(partners_[i]->episodeBuffer_->popTransition());
    }
    taus.push_back(episodeBuffer_->popTransition());
    lastEpisode_ = stackForVDN(taus);
  } else {
    lastEpisode_ = episodeBuffer_->popTransition();
  }
...
void RLActor::storeTrajectory(const HanabiEnv& env) {
  if (!env.terminated()) {
    return;
  }
