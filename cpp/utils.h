...
// inline infra::TensorDict convertSad(
...
inline infra::TensorDict observe(const hle::HanabiState& state, int playerIdx) {
  return observe(
      state,
      playerIdx,
      false,
...
std::tuple<infra::TensorDict, std::vector<int>, std::vector<float>> searchObserve(
    const hle::HanabiState& state, int playerIdx);
...
inline infra::TensorDict getH0(infra::BatchRunner& runner, int batchsize) {
...
infra::TensorDict applyModel(
    const infra::TensorDict& obs,
    infra::BatchRunner& runner,
    infra::TensorDict& hid,
...
void addHid(infra::TensorDict& to, infra::TensorDict& hid);

void moveHid(infra::TensorDict& from, infra::TensorDict& hid);

infra::TensorDict getResultAndErase(
  std::string key, std::unordered_map<std::string, infra::FutureReply>& map);
